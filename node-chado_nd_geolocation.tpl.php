<?php
// Purpose: This template provides the layout of the nd_geolocation node (page)
//   using the same templates used for the various nd_geolocation content blocks.
//
...
if (is_array($template_settings['tripal_no_sidebar']) and 
   $template_settings['tripal_no_sidebar']['nd_geolocation']) {
  $no_sidebar = 1;
}
...
<?php if ($teaser) { 
  print theme('tripal_nd_geolocation_teaser',$node); 
} else { ?>

<script type="text/javascript">
   Drupal.behaviors.nd_geolocationBehavior = function (context){      
      <?php if($no_sidebar){ ?>    
      ...
      $(".tripal-info-box-title").each(function(){
        ...
        var title = $(this).text();
        $('#tripal_nd_geolocation_toc_list').append('<li><a href="#'+id+'" class="tripal_nd_geolocation_toc_item">'+title+'</a></li>');
      });

      // when a title in the table of contents is clicked, then
      // show the corresponding item in the details box
      $(".tripal_nd_geolocation_toc_item").click(function(){
         $(".tripal-info-box").hide();
         href = $(this).attr('href');
         $(href).fadeIn('slow');
         // we want to make sure our table of contents and the details
         // box stay the same height
         $("#tripal_nd_geolocation_toc").height($(href).parent().height());
         return false;
      }); 
      ...
      if(block != null){
         $("#tripal_nd_geolocation-"+block[1]+"-box").show();
      } else {
         $("#tripal_nd_geolocation-base-box").show();
      }

      $("#tripal_nd_geolocation_toc").height($("#tripal_nd_geolocation-base-box").parent().height());
   };
</script>

<div id="tripal_nd_geolocation_details" class="tripal_details">

   <!-- Basic Details Theme -->
   <?php print theme('tripal_nd_geolocation_base',$node); ?>
   
   <!-- Properties Theme -->
   <?php print theme('tripal_nd_geolocation_properties',$node); ?>

   <!-- Resource Blocks CCK elements --><?php
   for($i = 0; $i < count($node->field_resource_titles); $i++){
     if($node->field_resource_titles[$i]['value']){ ?>
       <div id="tripal_nd_geolocation-resource_<?php print $i?>-box" class="tripal_nd_geolocation-info-box tripal-info-box">
         <div class="tripal_nd_geolocation-info-box-title tripal-info-box-title"><?php print $node->field_resource_titles[$i]['value'] ?></div>
...
<!-- Table of contents -->
<div id="tripal_nd_geolocation_toc" class="tripal_toc">
   <div id="tripal_nd_geolocation_toc_title" class="tripal_toc_title">Resources</div>
   <ul id="tripal_nd_geolocation_toc_list" class="tripal_toc_list">
